    ...
    for i in range(len(weight_specs)):
        ...
        if not compress:
            ...
            if DUMMY_WEIGHT not in filename:
                ...
                print(weight.shape, weight_specs[i])
            else:
...
class OutputEmbed:
    def __init__(self, config, env, policy):
        ...
        self.prefill_output_positions = torch.LongTensor([policy.prompt_len - 1]).to(self.compute.dev)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        # TODO: Move this to pytorch_backend
        exit(0)
        h = RMSNorm(h, w_ln)

        if i == 0:
            # Prefill phase
            output_positions = self.prefill_output_positions
        else:
            # Decoding phase
            output_positions = self.decode_output_positions

        embedder_weight = w_token
...
class SelfAttention:
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        w_qkv, w_out, w_ln, w_pln = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
                w_qkv.smart_copy(dst1), w_out.smart_copy(dst1),
                w_ln.smart_copy(dst2), w_pln.smart_copy(dst2)))
...
class MLP:
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        gate_proj, up_proj, down_proj = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((gate_proj, donate[1]), (up_proj, donate[2]), (down_proj, donate[3])) = weight_read_buf.pop()
        else:
            ((gate_proj, _), (up_proj, _), (down_proj, _)) = weight_read_buf.val

        h = self.compute.gemma_mlp(h, gate_proj, up_proj, down_proj, donate)
        hidden.val = h
